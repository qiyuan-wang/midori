...
var audio, bindButtonsEvents, bindKeyboardEvents, createPlayerDOM, current_track, decreaseVolume, decrease_vol, default_volume, duration, getAlbumName, getPerformers, increaseVolume, increase_vol, info, initPlayer, is_playing, loadTrack, load_progress, nextTrack, nextTrackAuto, next_song, play_button, play_pause, play_progress, play_section, previousTrack, previous_song, queryAlbum, removeTips, setProgress, single_loop, startMidori, switchLoop, switch_loop, tips, title, toggleMusic, total_tracks, tracklist, try_button, updateLoadProgress, updateProgress;
...
getPerformers = function() {
  ...
  $performers.each(function() {
    return performers.push(this.innerText.replace(/(original\s)?(motion picture\s)?soundtrack/i, "").replace(/various\s?artist(s)?/i, "").replace(/(^\s+|\s+$)/g, ''));
  });
  return performers;
};
...
bindKeyboardEvents = function() {
  return $(document).keypress(function(evt) {
    switch (evt.which) {
      case 32:
        evt.preventDefault();
        toggleMusic();
